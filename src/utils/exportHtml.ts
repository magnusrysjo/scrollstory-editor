...
export function exportStoryHtml(story: Story): void {
  ...
  const knownFonts = [
    ...
    findFont(captionFont, BODY_FONTS),
  ...
  if (isCustomFont(captionFont, BODY_FONTS)) {
    fontLinks.push(customFontExportMarkup(captionFont, theme.fontCaptionUrl));
  }
...
${fontLinks.join('\n')}
  <style>
    :root {
      ...
      --font-caption: ${theme.fontCaption ?? "'JetBrains Mono', monospace"};
      --color-primary: ${theme.colorPrimary};
      --color-text: ${theme.colorText};
    }

    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: var(--font-body); background: #f8f6f1; -webkit-font-smoothing: antialiased; }
    ...
    .block-caption { font-family: var(--font-caption); font-size: 0.75rem; letter-spacing: 0.05em; color: var(--color-text); opacity: 0.5; margin-bottom: 0.75rem; text-transform: uppercase; }
    ...
    blockquote.block-quote p { font-family: var(--font-heading); font-size: clamp(1.1rem,2.5vw,1.6rem); font-style: italic; color: var(--color-text); margin-bottom: 0.75rem; }
    blockquote.block-quote cite { font-family: 'JetBrains Mono', monospace; font-size: 0.72rem; color: var(--color-primary); font-style: normal; }

    .block-image { margin: 0 0 1.5rem; width: 100%; }
    .block-image img { width: 100%; height: auto; display: block; border-radius: 4px; }
    .block-image figcaption { font-family: var(--font-caption); font-size: 0.7rem; color: var(--color-text); opacity: 0.5; margin-top: 0.5rem; }
